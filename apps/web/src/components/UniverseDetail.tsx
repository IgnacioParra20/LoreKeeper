...
import { createCharacterSchema } from "@lorekeeper/validation";
import { ApiError, createCharacter, deleteCharacter, getCharacters, getUniverse, isAborted, updateCharacter } from "../api/client";
...
export const UniverseDetail = ({ universeId }: { universeId: string }) => {
  ...
  return <main className="main-content universe-detail">
    ...
    {loading ? <p role="status">Cargando universo…</p> : error && !universe ? <div className="empty-state"><h1>No pudimos abrir este universo</h1><p>{error}</p></div> : universe ? <>
      <div className="detail-heading"><div><p className="eyebrow">UNIVERSO NARRATIVO</p><h1>{universe.name}</h1><p>{universe.description || "Un mundo listo para tomar forma."}</p></div><span className="detail-status">{universe.status === "ACTIVE" ? "Activo" : "Archivado"}</span></div>
      <section className="character-section" aria-labelledby="characters-title">
        <div className="character-heading"><div><h2 id="characters-title">Personajes</h2><p>{characters.length} {characters.length === 1 ? "personaje" : "personajes"} en este universo</p></div><button className="button button--primary" type="button" onClick={() => { setEditing("new"); setFormError(null); }}>Crear personaje</button></div>
        ...
        {characters.length === 0 ? <div className="empty-state character-empty"><h3>Todavía no hay personajes</h3><p>Empieza por alguien que habite este mundo: quién es y qué papel cumple.</p></div> : <div className="character-list">{characters.map((value) => <article className="character-card" key={value.id}>
          <div className="character-avatar" aria-hidden="true">{value.name.slice(0, 1).toLocaleUpperCase("es")}</div><div className="character-copy"><h3>{value.name}</h3>{value.role ? <p className="character-role">{value.role}</p> : null}<p>{value.description || "Sin descripción todavía."}</p></div>
